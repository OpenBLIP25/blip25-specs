...
13. [AMBE-3000 Rate-Index Table: Bit-Allocation and Naming Gaps](ambe3000_rate_bit_allocation_gap.md) — two coupled gaps surfaced while producing `halfrate_bit_allocations.csv` and `rate_conversion_pairs.csv`: (1) per-field bit allocations for AMBE+2 chip rates 35–61 are not in public sources, requiring empirical characterization in `~/blip25-mbe`; (2) project-wide `r33`/`r34` aliases (P25 full-rate / half-rate) collide numerically with chip rate indices 33/34 (which are both half-rate variants). New CSVs follow chip indexing with P25 full-rate at project-extension rows 62/63.
